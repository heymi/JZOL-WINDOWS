  ...
  $("#dataGrid").bootgrid({
        ...
        url: function () {
          ...
          if (searchPhrase.length) {
            return CONFIG.API_DOMAIN + "/DemandsBiddings/myRecommend";
          } else {
            return CONFIG.API_DOMAIN + "/DemandsBiddings/myRecommend";
          }
        },
        ...
        formatters: {
          "operations": function (column, row) {
            ...
            switch (status) {
              case 0:
              case '0':
                return "<button type=\"button\" class=\"btn-yellow-small\" onClick=\"location.href='order-wait.html?orderId=" + row.id + "&noshare=1'\">匹配阿姨</button>";
              case 1:
              case '1':
                return "<button type=\"button\" class=\"btn-blue-small\" onClick=\"location.href='order-setup.html?orderId=" + row.id + "'\">查看详情</button>";
              case 2:
              case '2':
                return "<button type=\"button\" class=\"btn-green-small\" onClick=\"location.href='order-setup.html?orderId=" + row.id + "'\">查看详情</button>";
              case 5:
              case '5':
          ...
          "status": function (column, row) {
            ...
            if (row.bills && row.bills.length) {
              bill_status = row.bills[row.bills.length - 1].bill_status;
            }
            var status = row.bill_status || row.status;
            switch (status) {
              case 0:
